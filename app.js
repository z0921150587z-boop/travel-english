...
function cloudSave(){if(!sb||!CURRENT_UID)return;clearTimeout(saveTimer);saveTimer=setTimeout(async()=>{try{
  ...
  await sb.from('profiles').upsert({id:CURRENT_UID,last_active:new Date().toISOString(),xp:S.xp,streak:streak(),lessons_done:Object.values(S.done).filter(v=>v>=0).length,days_done:Object.keys(S.done).length,dest_name:S.dest?S.dest.name:null,retention:retention().acc});
...
async function cloudLoad(){if(!sb||!CURRENT_UID)return;try{const {data}=await sb.from('progress').select('state').eq('user_id',CURRENT_UID).maybeSingle();
  if(data&&data.state&&(data.state.updatedAt||0)>(S.updatedAt||0)){S=Object.assign(S,data.state);sanitize();try{localStorage.setItem(KEY+':'+CURRENT_UID,JSON.stringify(S));}catch(e){}}
...
/* ===== SESSION ===== */
let sess=null;
function startLesson(li,practice){if(!LESSONS[li])return home();const P=plan();const due=practice?[]:dueItems().slice(0,P.reviewCap);
  sess={li,practice,review:due,words:items(li),wi:0,si:0,prodEarly:P.prodEarly,dest:(!practice&&S.dest)?pickDest(4):[]};stepScene();}
function startReviewOnly(){const P=plan();const due=dueItems().slice(0,14);if(!due.length)return home();sess={li:null,practice:true,review:due,words:[],reviewOnly:true,prodEarly:P.prodEarly};stepReview();}
function startDestDrill(){if(!S.dest||!S.dest.items||!S.dest.items.length)return destScreen();sess={li:null,practice:true,review:[],words:[],destOnly:true,dest:pickDest(8)};stepDest();}
function pickDest(k){if(!S.dest)return [];const ids=S.dest.items.map(x=>x.id).filter(id=>ALL[id]);const t=todayStr();
...
function startConsolidate(){const P=plan();const weak=weakest(6).map(x=>x.id).filter(id=>ALL[id]);const due=dueItems().slice(0,P.reviewCap);
  if(!weak.length&&!due.length)return home();
...
function stepScene(){currentScreen='scene';const L=LESSONS[sess.li];if(!L)return stepReview();const ss=sess.words.filter(x=>x.type==='s');
  h(`${stepbar('情境對話',0.05)}
...
function playScene(){if(!('speechSynthesis' in window)||!sess)return;speechSynthesis.cancel();sess.words.filter(x=>x.type==='s').forEach(s=>{const u=new SpeechSynthesisUtterance(s.en);u.lang='en-US';if(voice)u.voice=voice;u.rate=0.85;speechSynthesis.speak(u);});}
function stepReview(){currentScreen='review';if(!sess.review.length){afterReview();return;}
  const qs=sess.review.filter(id=>ALL[id]).map(id=>makeQ(ALL[id],'review',sess.prodEarly));
  if(!qs.length){afterReview();return;}
  runQuiz('複習',qs,'review',(results)=>{results.forEach(r=>grade(r.id,r.ok));save();sess.reviewResults=results;afterReview();});}
...
function stepDest(){currentScreen='dest-drill';if(!S.dest){if(sess.destOnly)return home();return stepWords();}const ids=(sess.dest||[]).filter(id=>ALL[id]);
  if(!ids.length){if(sess.destOnly)return home();return stepWords();}
  const fresh=ids.filter(id=>!S.srs[id]);
  const qs=ids.map(id=>makeQ(ALL[id],S.srs[id]?'review':'new',false));
  if(fresh.length&&!sess.destShown){sess.destShown=true;
    h(`${stepbar('目的地加強 · '+esc(S.dest.name),0.2)}<section class="card"><div class="eyebrow coral">${esc(S.dest.name)} 專屬</div>
...
function stepWords(){currentScreen='words';const ws=sess.words.filter(x=>x&&x.type==='w');if(!ws.length){stepSents();return;}
  if(sess.wi>=ws.length)sess.wi=ws.length-1;const w=ws[sess.wi];const n=ws.length;
  const title=sess.consolidate?'弱點重學':esc(lessonName(sess.li));
  ...
  <div class="row">${sess.wi>0?`<button class="btn ghost" onclick="sess.wi--;stepWords()">上一個</button>`:''}<button class="btn" onclick="${sess.wi<n-1?'sess.wi++;stepWords()':'stepSents()'}">${sess.wi<n-1?'下一個':(sess.words.some(x=>x&&x.type==='s')?'學句子':'小測驗')}</button></div>`);
  if(S.autoplay)setTimeout(()=>say(w.en),250);}
function stepSents(){currentScreen='sents';const ss=sess.words.filter(x=>x&&x.type==='s');if(!ss.length){stepQuiz();return;}
  if(sess.si>=ss.length)sess.si=ss.length-1;const s=ss[sess.si];const n=ss.length;
...
function stepQuiz(){currentScreen='quiz';const pool=shuffle(sess.words.filter(Boolean)).slice(0,6);const ctx=sess.consolidate?'review':'new';
  if(!pool.length){finish([...(sess.reviewResults||[]),...(sess.destResults||[])],[]);return;}
...
function setDest(slug){const pack=DEST_PACKS[slug];if(!pack)return;S.dest={slug,name:pack.name,tips:pack.tips||[],items:destItems(pack,slug),createdAt:Date.now()};rebuildAll();save();destScreen();}
function clearDest(){if(S.dest&&S.dest.items){S.dest.items.forEach(it=>{delete S.srs[it.id];});}S.dest=null;rebuildAll();save();destScreen();}
...
function shareScreen(){currentScreen='share';const u=appUrl();
  ...
  <button class="btn ghost" onclick="home()">回首頁</button>`,'home');
  try{new QRCode(document.getElementById('qr'),{text:u,width:180,height:180,colorDark:'#0B1026',colorLight:'#ffffff'});}catch(e){const el=document.getElementById('qr');if(el)el.style.display='none';}}
function copyUrl(){const u=appUrl();(navigator.clipboard?navigator.clipboard.writeText(u):Promise.reject()).then(()=>{const el=document.getElementById('copied');if(el)el.textContent='已複製連結';},()=>{prompt('複製這個連結',u);});}
